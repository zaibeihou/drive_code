...
static int module_cdev_init(void)
{
    int ret;
// 1. 申请设备号
    ret = alloc_chrdev_region(&dev1.dev_num, 0, 2, "atomic_test");
    if(ret != 0){
...
// 4. 创建一个class，用于两个设备
    dev1.class_test = class_create(THIS_MODULE, "atomic_test");
    if(IS_ERR(dev1.class_test)){
        ret = PTR_ERR(dev1.class_test);//注意分辨 PTR_ERR 和 ERR_PTR
        goto class_create_error;
    }
// 5. 创建两个不同名字的设备节点
    dev1.device = device_create(dev1.class_test, NULL, dev1.dev_num, NULL, "atomic_test0");
    if(IS_ERR(dev1.device)){
        ret = PTR_ERR(dev1.device);
        goto device_create1_error;
    }
    dev2.device = device_create(dev1.class_test, NULL, dev2.dev_num, NULL, "atomic_test1");
    if(IS_ERR(dev2.device)){
        ret = PTR_ERR(dev2.device);
        goto device_create2_error;
    }

    printk("主设备号：%d 次设备号:test0 %d  test1  %d\n",dev1.major,dev1.minor,dev2.minor);
    return 0;
