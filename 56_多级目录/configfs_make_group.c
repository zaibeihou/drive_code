...
// 必须包含一个config_group作为第一个成员
struct mygroup {
    struct config_group item;   // 继承自config_group
    int size;
    void *addr;
    // 这里可以添加自己的其他成员
};
...
void mygroup_drop(struct config_group *group, struct config_item *item){
    struct myitem *myitem;
...
static const struct config_item_type mygroup_type = {
    .ct_owner = THIS_MODULE,
};

struct config_group *mygroup_makegroup(struct config_group *group, const char *name){
    struct mygroup *mygroup;
    mygroup = kzalloc(sizeof(*mygroup), GFP_KERNEL);
    config_group_init_type_name(&mygroup->item, name, &mygroup_type);
    return &mygroup->item;
};
...
struct configfs_group_operations mygroup_ops = {
    ...
    .drop_item = mygroup_drop,               // 指定删除item的处理函数
    .make_group = mygroup_makegroup,
};
