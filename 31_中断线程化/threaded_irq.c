...
irqreturn_t work_func(int irq, void *args){
    msleep(1000);
...
static int interrupt_init(void)
{
    ...
    irq = gpio_to_irq(101);
    ret = request_threaded_irq(irq,interrupt_func,work_func,IRQF_TRIGGER_RISING,"interrupt_test",NULL);
    if(ret < 0){
    ...
    //work_queue =  alloc_workqueue("work_queue",WQ_UNBOUND,0);
